...
struct btree *load_database(const char *filename, int start, int end);
...
int main(int argc, char **argv) {
  ...
  if (argc >= 3) {
	...
	if (rank==0)
	{
		long lines = count_lines(filename);
		long lines_per_proc = lines/size;
		long remainder = lines%size;
		long lines_start* = malloc(size * sizeof(long));
		long lines_end* = malloc(size * sizeof(long));
		int total = 0;
...
for (int 
    ...
    scanned = fscanf(fp, "%d %19s %d %19s %d %19s", &car.ID, car.Model, &car.YearMake, car.Color, &car.Price, car.Dealer);
...
static int compare_attr_value(const CarInventory *car, const char *attr,
  ...
  if (strcasecmp(attr, "ID") == 0 || strcasecmp(attr, "YearMake") == 0 ||
    ...
    if (lhs < rhs) return -1;
    if (lhs > rhs) return 1;
    return 0;long count_lines(const char *filename) {
    FILE *fp = fopen(filename, "r");
    if (!fp) return -1;
